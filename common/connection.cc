...
map<const Configuration*, Connection*> Connection::_instances;
const int error_socket = std::numeric_limits<int>::min();

Connection* Connection::GetInstance(const Configuration& config){
    map<const Configuration*, Connection*>::iterator it =
        _instances.find(&config);
    if(it == _instances.end())
        it = _instances.insert(std::make_pair(
                    &config, new Connection(config))).first;
    return it->second;
}
...
void Connection::AddConnection(const Node* node, int fd){
    switch(node->nodeType){
    case DB_NODE: {
...
void Connection::InitializeDB(const DBNode* node){
...
void Connection::InitializePreprocessor(const PreprocessorNode* node){
...
void Connection::InitializeMediator(const MediatorNode* node){
    _server_sock_txns = prepare_serversock(node->port);
    if(_server_sock_txns < 0)
        exit(0);
...
void Connection::AcceptingConnection(int server_sock){
...
int Connection::ConnectionEstablished(fd_set* fds){
    // Type: [fd, nodeID]
...
void Connection::FillIncomingTxns(
...
void Connection::FillIncomingMessages(CircularBuffer<Message>* msgs){
    static int retryCount;
...
int Connection::ReadTxn(int fd, GenericTxn* txn){
...
int Connection::ReadMsg(int fd, Message* msg){
    // Reliable read pattern
...
void Connection::Disconnected(int node_id){
...
ssize_t Connection::Send(int node_id, const void* buf,
                                      size_t count) const{
...
ssize_t Connection::SendReplicasAndPartitions(
...
ssize_t Connection::SendDB(int replica, int partition, const void* buf,
                                 size_t count){
    return SendReplicasAndPartitions(_db_fds, replica, partition, buf, count);
}

ssize_t Connection::SendDBMsg(const Message& message){
...
ssize_t Connection::SendPreprocessor(int replica, int partition,
                                           const void* buf, size_t count){
    return SendReplicasAndPartitions(_preproc_fds, replica, partition,
                                     buf, count);
}

ssize_t Connection::SendMediator(int partition, const void* buf,
                                       size_t count){
